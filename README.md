...
- **[studylife-focusguard](https://github.com/lukislp/studylife-focusguard)** — Chrome extension (Manifest V3) that blocks or allows sites while a StudyLife focus session is running (allowlist or blocklist), with automatic tab redirect on session start and restore on session end. Its API key is read-only by design — it can only ever check whether a session is active, nothing else. TypeScript + esbuild.
